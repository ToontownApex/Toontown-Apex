# Embedded file name: .\building\DistributedSuitInterior.py
...
from toontown.minigame import PurchaseBase
...
class DistributedSuitInterior(DistributedObject.DistributedObject):
    ...
    def __playElevator(self, ts, name, callback):
        ...
        if self.currentFloor == 999:
            self.cou.setText('Floor ' + str(self.defaultcou))
        ...
        elif self.currentFloor == 10:
            self.cou.setText('Floor ' + str(int(self.currentFloor) + 1))
            self.floorModel = loader.loadModel('phase_11/models/lawbotHQ/LB_Zone04a.bam')
            SuitHs = self.BottomFloor_SuitHs
            SuitPositions = self.BottomFloor_SuitPositions
        else:
            self.cou.setText('Floor ' + str(int(self.currentFloor) + 1))
